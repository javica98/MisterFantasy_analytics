...
def procesar_clausulas_acuerdos(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df_filtrado = df_filtrado.drop(["type","mensaje","posicionJugador","puntosJugador","equipoLiga","name","money","position","aciertos","points","jornada","idTransfer"], axis=1, errors='ignore')
    return df_filtrado
